...
class MainActivity : AppCompatActivity() {
    ...
    private fun downloadPhoto(photoUrl: String?){
        photoUrl ?: return // 유효하지 않은 url은 그냥 반환

        Glide.with(this)
            .asBitmap()
            .load(photoUrl)
            .diskCacheStrategy(DiskCacheStrategy.NONE) // 여기선 굳이 캐싱 전략을 이용해서 빠르게 다운받을 필요 x
            .into(
                object : CustomTarget<Bitmap>(SIZE_ORIGINAL, SIZE_ORIGINAL){
                    ...
                    override fun onLoadFailed(errorDrawable: Drawable?) { // 불러오는게 취소돼서 리소스가 해제됐을 때
